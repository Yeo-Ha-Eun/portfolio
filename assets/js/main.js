 $(document).ready(function () {
    ...
    // modal window2 : portfolio
    $('#portfolio .bottom button').on('click', function (e) {
      //1) 필요한 변수 선언
      ...
      var _horSlide = _mdCnt2.find('.pj-container'); //열려진 모달창에서 가로 스크롤 대상
      ...
      _closeBtn2.on('click', function () {
        ...
        _mdCnt2.stop().animate({opacity: 0}, function () {
          ...
          // 3) 가로 스크롤 초기 상태로 되돌리기 - 추가
        _horSlide.find('.indicator ul li').eq(0).children().trigger('click');
        });
      ...
      // 가로 스크롤 함수 호출 => 동작 대상 매개변수로 전달하기
      horizontalScroll(_horSlide);
    });
  
    //가로 스크롤 함수 - 추가
    function horizontalScroll(_horTarget) {
      var _win = $(window);
      var _menu = _horTarget.find('.indicator ul li');
      var _cntWrap = _horTarget.find('.pj_wrap');
      var tgIdx = 0;  //로딩시 보여지는 섹션의 인덱스 번호, 인디케이터의 활성화 번호
      var total = _cntWrap.children().size(); //섹션의 전체 개수
      var winWidth;   //window의 가로크기를 저장할 전역변수
      var timerWheel = 0; //mousewheel 이벤트의 실행문 누적을 방지
      console.log(_horTarget, total, tgIdx);
      ...
      winWidth = _win.width();
      _cntWrap.css('width', winWidth * total).children().css('width', winWidth);
      ...
      _menu.children().on('click', function (e) {
          ...
          if ( _cntWrap.is(':animated') ) return false;
          tgIdx = $(this).parent().index(); //인디케이터 li의 인덱스번호
          //console.log(tgIdx);
          //3-2) 클릭한 인디케이터가 활성화
          $(this).parent().addClass('on').siblings().removeClass('on');
          //3-3) 애니메이트(.cnt_wrap)
          _cntWrap.stop().animate({marginLeft: tgIdx * winWidth * -1}, 700);
      });
    
     //4) 마우스휠 이벤트
      _cntWrap.on('mousewheel DOMMouseScroll', function (e) {
        ...
        timerWheel = setTimeout(function () {
          ...
          _menu.eq(tgIdx).addClass('on').siblings().removeClass('on');
          _cntWrap.stop().animate({marginLeft: tgIdx * winWidth * -1}, 700);
        }, 200);
     });
     ...
     _horTarget.find('.controler button').on('click', function (e) {
        //5-1) 현재 애니메이트(.cnt_wrap) 중이면 함수 강제 종료
        if ( _cntWrap.is(':animated') ) return false;
